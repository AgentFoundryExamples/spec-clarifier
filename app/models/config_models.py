...
class ClarificationConfig(BaseModel):
    ...
    model: str | None = Field(
        default=None, min_length=1, description="Model identifier specific to the provider"
    )
    system_prompt_id: str | None = Field(
        default=None, min_length=1, description="Identifier for the system prompt template to use"
    )
    temperature: float | None = Field(
        default=None, ge=0.0, le=2.0, description="Sampling temperature for response generation"
    )
